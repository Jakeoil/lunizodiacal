...
// SVG dimensions and mapping — wider, more square
const svgW = 500, svgH = 600;
const padX = 80, padY = 60; // room for symbols
...
for (const sp of seasonPoints) {
  ...
  svg += `  <circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${symbolSize * 0.7}" fill="rgba(0,0,0,0.6)" stroke="${SIGN_COLOR[sp.sign]}" stroke-width="2"/>\n`;
